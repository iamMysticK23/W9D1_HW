...
const UpdateQuantity = (props: UpdateState) => {
  ...
  const onSubmit: SubmitHandler<SubmitState> = async (data, event) => {
    if (event) event.preventDefault(); // if an event is happening or we take in event, it stops the default (like a refresh screen)
    ...
    if (response.status === 200) {
      setMessage("Successfully updated car in your purchase order");
...
export const Order = () => {
  ...
  const [gridData, setGridData] = useState<GridRowSelectionModel>([]); // hook to set the row in grid as current object so we can edit
  const [openAlert, setAlertOpen] = useState(false);
  const [message, setMessage] = useState<string>();
  const [messageType, setMessageType] = useState<MessageType>();
  ...
  const deleteItem = async () => {
    ...
    for (let order of orderData) {
      // looping thru the list of order objects
    ...
    if (response.status === 200) {
      setMessage("Successfully deleted car from purchase order");
      setMessageType("success");
      setAlertOpen(true);
      setTimeout(() => {
        window.location.reload();
