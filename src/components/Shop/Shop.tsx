  ...
  const onSubmit: SubmitHandler<SubmitState> = async (data, event) => {
    ...
    push(cartRef, myCart)
      .then((newCartRef) => {
        ...
        setMessage(`Successfully added ${myCart.name} to Cart`);
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Box>
          <label htmlFor="quantity">
            Input number of {cart.cartItem.name} you wish to purchase
          </label>
          <InputText
            {...register("quantity")}
            name="quantity"
...
export const Shop = () => {
  ...
  // I wanted a way to show a car as "Sold Out" once the quantity hit 0
  // this makes the card greyed out and non clickable
  const soldOutCar = {
    opacity: 0.5,
    pointerEvents: "none"
}
  ...
  return (
    <Box sx={shopStyles.main}>
      ...
      <Typography variant="h4" sx={shopStyles.typography}>
        Cars Avaiable For Purchase
      </Typography>
      <Grid container spacing={4} sx={shopStyles.grid}>
        {shopData.map((shop: ShopState, index: number) => (
          <Grid item key={index} xs={12} sm={6} md={4} style={shop.quantity === 0 ? (soldOutCar as CSSProperties) : {}}> {/* show a car as sold out by greying it out */}
            <Card sx={shopStyles.card}>
              ...
              <CardContent>
                <Typography>
                  <strong>
                    <center>Quantity Available: {shop.quantity}</center>
                  </strong>
                </Typography>
                <Stack
                  direction="column"
